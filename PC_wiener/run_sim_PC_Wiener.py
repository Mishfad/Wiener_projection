...
def continuous_data_generation(nSamples,B,A,noise_pow):
    nNodes=B.shape[0]
    # print(B)
    y=np.zeros((nSamples,nNodes),dtype=complex)
    x=np.zeros((nSamples,nNodes))
    x[0,:]=np.random.randn(nNodes)*np.sqrt(noise_pow)
    x[1,:]=np.random.randn(nNodes)*np.sqrt(noise_pow)-A[:,0]*x[0,:]
    x[2,:]=np.random.randn(nNodes)*np.sqrt(noise_pow)-A[:,0]*x[1,:]-A[:,1]*x[0,:]
    for ind_Samp in range(3,nSamples):
        ...
        # print("Shape: ",X.shape)
        # print("\n\nfft:",X)
        # y[:,:]=np.transpose(X) #X is (nFreq,nSamples)
        # print("y:",y[ind_Traj,:,:])
    return x
##############################################################################################################################        
...
ax=plt.gca()
ax.set_title("Estimated topology by PC-Wiener")
plt.savefig('/Users/mishfad/Google Drive/My Drive/UMN/PhD/Simulations/simulation-python/Python_codes/Wiener_projection_simulation/PC_Wiener/Top_est_PC_Wiener.pdf',format='pdf')
plt.show()
