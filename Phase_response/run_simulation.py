...
# plot the phase response of complex matrix W
    ...
    fig1.savefig('/Users/mishfad/Library/CloudStorage/GoogleDrive-veedu002@umn.edu/My Drive/UMN/PhD/Simulations/simulation-python/Python_codes/Wiener_projection_simulation/working_phase/phase_response.pdf',format='pdf')
    plt.show(block=False)
...
def create_A_B_coeffs(Adj,filter=None):
    ...
    # A=np.ones((nNodes,3))
    A=np.array([[1, .6,.3],[1, .4,.2],[1, .7,.4],[1, .5,.3],[1, .6,.3],[1, .4,.2] ])
    ...
    B=B*Adj
    B[:,:,1:3]=0
    # np.savez('/Users/mishfad/Google Drive/My Drive/UMN/PhD/Simulations/simulation-python/Python_codes/Wiener_projection_simulation/working_phase/AB_coeffs',A=A,B=B.reshape(B.shape[0], -1))
    return A,B
